...
#ifdef WIN32
	#define ctp_helper_trade_API __declspec(dllexport)
#else
	#define ctp_helper_trade_API __attribute__((visibility("default"))) 
#endif

typedef struct TradeHelperData TradeHelperData ;

class ctp_helper_trade_API TraderApiHelper : public CThostFtdcTraderApi
{
public:
	///创建TraderApi
...
public:    
    TraderApiHelper();
	virtual ~TraderApiHelper();

private:
    struct TradeHelperData * m_data;
};
